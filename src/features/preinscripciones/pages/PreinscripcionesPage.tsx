    ...
    <main>
      ...
      <section className="mx-auto max-w-4xl px-5 py-14 sm:px-8">
        <form onSubmit={submit} className="border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <label className="mt-5 block text-sm font-bold text-[#171717]">Carrera<select required value={form.carreraId} onChange={(event) => update("carreraId", event.target.value)} className="mt-2 w-full border border-slate-300 bg-white px-3 py-3 font-normal outline-none focus:border-[#C49200]"><option value="">Seleccioná una carrera</option>{carreras.carreras.map((carrera) => <option key={carrera.id} value={carrera.id}>{carrera.nombre}</option>)}</select></label>
          {carreras.cargando && <p className="mt-3 text-sm text-slate-500">Cargando carreras...</p>}
          {carreras.error && <p className="mt-3 text-sm text-amber-700">No pudimos cargar las carreras. Intentá nuevamente más tarde.</p>}
          ...
          <button type="submit" disabled={mutation.isPending} className="mt-6 inline-flex items-center gap-2 bg-[#171717] px-5 py-3 font-bold text-white hover:bg-[#C49200] disabled:opacity-60">{mutation.isPending ? "Enviando..." : "Enviar preinscripción"}<Send size={17} /></button>
        </form>
      </section>
    </main>
  );
