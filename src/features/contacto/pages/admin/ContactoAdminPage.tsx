...
import { useContactAdminActions, useContactMessages } from "../../hooks/useContact";
...
export default function ContactoAdminPage() {
  ...
  if (messagesQuery.isError) {
    ...
    return <main className="mx-auto max-w-7xl px-5 py-12"><ErrorState title={unauthorized ? "Tu sesión venció" : "No pudimos cargar los mensajes"} description={unauthorized ? "Volvé a iniciar sesión para consultar la bandeja de contacto." : undefined} onRetry={() => unauthorized ? navigate("/login") : void messagesQuery.refetch()} retryLabel={unauthorized ? "Ir al inicio de sesión" : undefined} /></main>;
  }
  ...
  return <main className="mx-auto max-w-7xl space-y-8 px-5 py-8 sm:px-8 lg:px-12"><header><p className="text-sm font-bold uppercase tracking-[0.18em] text-[#C49200]">Administración</p><h1 className="mt-2 text-3xl font-black text-[#171717]">Contacto</h1><p className="mt-2 text-slate-600">Gestioná las consultas recibidas por el formulario público.</p></header><section className="space-y-4"><MessageFilters search={search} status={status} onSearch={setSearch} onStatus={setStatus} /><MessageTable messages={messages} onSelect={setSelected} /></section><MessageDetailDrawer message={selected} busy={actions.status.isPending || actions.reply.isPending} onClose={() => setSelected(null)} onStatus={(value) => void changeStatus(value)} onReply={reply} /></main>;
}
