...
const fallbackInformation = { email: "", telefono: "", direccion: "", horario: "" };

export default function ContactoPage() {
  const { data, isLoading, isError } = useContactInformation();
  if (isLoading) return <div className="grid min-h-[60vh] place-items-center"><LoadingSpinner size="mediano" text="Cargando información de contacto..." /></div>;
  if (isError) {
    console.warn("La información de contacto no está disponible; se muestran datos de respaldo.");
  }
  const information = data ?? fallbackInformation;
