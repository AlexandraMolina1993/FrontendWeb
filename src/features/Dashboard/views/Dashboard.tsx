...
import { CalendarDays, ClipboardList, GraduationCap, Users, Newspaper, Image } from "lucide-react";
        ...
        {!esBiblioteca && (
          <nav aria-label="Accesos rápidos" className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                { titulo: "Carreras", ruta: "/admin/carreras", icono: GraduationCap },
                { titulo: "Usuarios", ruta: "/admin/usuarios", icono: Users },
                { titulo: "Noticias", ruta: "/admin/noticias", icono: Newspaper },
                { titulo: "Galería", ruta: "/admin/galeria", icono: Image },
              ].map(({ titulo, ruta, icono: Icono }) => (
                <Link key={ruta} to={ruta} className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm font-bold text-zinc-800 shadow-sm transition hover:border-[#E4B600] hover:bg-amber-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-600">
                  <Icono size={20} className="shrink-0 text-[#8A6700]" aria-hidden="true" />
                  {titulo}
                </Link>
              ))}
            </div>
