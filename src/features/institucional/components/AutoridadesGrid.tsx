...
export default function AutoridadesGrid({ autoridades }: { autoridades: Autoridad[] }) {
  ...
  return <div className="grid gap-5 md:grid-cols-2">{autoridades.slice().sort((a, b) => a.orden - b.orden).map((autoridad) => <AutoridadCard key={autoridad.id} autoridad={autoridad} />)}</div>;
}