...
export default function AutoridadesGrid({ autoridades }: { autoridades: Autoridad[] }) {
  ...
  return <div className="grid gap-5 md:grid-cols-2">{autoridades.slice().sort((a, b) => (a.orden ?? 0) - (b.orden ?? 0)).map((autoridad) => <AutoridadCard key={autoridad.id} autoridad={autoridad} />)}</div>;
}