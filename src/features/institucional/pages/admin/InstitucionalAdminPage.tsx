...
export default function InstitucionalAdminPage() {
  ...
  async function guardarAutoridad() { const errores = validarAutoridad(autoridad); if (Object.keys(errores).length) { window.alert(Object.values(errores)[0]); return; } if (!sedeId || !informacion.id) { window.alert("Guardá primero la información institucional de la sede."); return; } setGuardando(true); try { const actualizada = await institucionalApi.actualizar(sedeId, { ...informacion, autoridades: [...autoridades, autoridad] }); setInformacion(actualizada); setAutoridades(actualizada.autoridades); setAutoridad(AUTORIDAD_FORM_VACIO); } catch (err) { window.alert(getApiErrorMessage(err)); } finally { setGuardando(false); } }
